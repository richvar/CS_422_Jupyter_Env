...
richard_pkl_path = '/home/s25vargason1/richard/512_basic_training/stylegan2-ada-pytorch/results/00008-post-impressionist-auto1-ada-resumecustom/network-snapshot-000900.pkl'
...
truncation_psi = 0.6  # Lower values -> more similarity, higher values -> more diversity
...
num_random_images = 25  # Number of images to generate if seeds list is empty (make a 5x5 grid)
...
# --- Create list to hold generated images ---
images = []
...
grid_img = make_grid([image.permute(2, 0, 1).float() / 255.0 for image in images], nrow=5)

# --- Save the grid as a single image ---
save_path = os.path.join(output_dir, 'image_grid.jpg')
save_image(grid_img, save_path)

print(f'✅ Saved image grid at {save_path}')
