...
get_ipython().system('python3 -m ipykernel install --user --name=stylegan2env --display-name "Python (StyleGAN2)"')
...
get_ipython().system('git clone https://github.com/NVlabs/stylegan2-ada-pytorch.git')
get_ipython().run_line_magic('cd', 'stylegan2-ada-pytorch')
...
if expected not in os.path.basename(cwd):
    ...
    get_ipython().system('pip install torch==1.10.1 torchvision==0.11.2')
    get_ipython().system('pip install click requests tqdm pyspng ninja imageio-ffmpeg scipy')
...
def convert_images_for_stylegan2ada_pytorch(
    ...
    get_ipython().system('LC_ALL=C.UTF-8 LANG=C.UTF-8 python3 dataset_tool.py --source=../images/full --dest=./datasets/post-impressionist')
...
get_ipython().system('pip install --user psutil')
...
# Start training
get_ipython().system('python3 train.py    --outdir=./results    --data=./datasets/post-impressionist    --gpus=1    --snap=1    --resume=ffhq1024    --cfg=auto    --aug=ada | tee {log_file}')
